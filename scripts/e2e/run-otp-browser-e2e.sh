#!/usr/bin/env bash
# SPABLA V2 · Fase 9 · Hito 9.3.2-B-Q3 · Runner for the real OTP
# browser E2E barrier.
#
# Mirror of `scripts/e2e/run-onboarding-e2e.sh`, isolated on port
# 3131 so it can coexist with the other two E2E runners
# (auth-continuity 3111, onboarding 3121). Boots:
#
#   1. Supabase local (idempotent — reuses if already up; else
#      starts and marks for cleanup).
#   2. Migration chain (`--reset` in CI).
#   3. Waits for PostgREST readiness.
#   4. `next dev` on `SPABLA_E2E_NEXT_PORT` (default 3131).
#   5. Waits for Next dev readiness.
#   6. `npx playwright test e2e/otp-signin.spec.ts --project chromium`.
#   7. Cleanup: kill Next, kill residual Chromium, release port.
#      If runner started Supabase → stop it; if it was pre-existing
#      → leave it running (custody policy Q3-R).
#
# Usage:
#   scripts/e2e/run-otp-browser-e2e.sh [--reset]
...
for arg in "$@"; do
  case "$arg" in
    --reset) RESET=1 ;;
    *) echo "[run-otp-e2e] unknown flag: $arg" >&2; exit 2 ;;
  esac
done
...
E2E_NEXT_PORT="${SPABLA_E2E_NEXT_PORT:-3131}"
E2E_BASE_URL="http://127.0.0.1:${E2E_NEXT_PORT}"
NEXT_PID=""
NEXT_PGID=""
NEXT_LOG="$(mktemp -t spabla-e2e-otp-next.XXXXXX)"
CUSTODY_LOG="${SPABLA_E2E_CUSTODY_LOG:-$(mktemp -t spabla-e2e-otp-custody.XXXXXX)}"
SUPABASE_STARTED_BY_RUNNER=0

log() { printf '[e2e-otp] %s\n' "$*" >&2; }
...
_snapshot() {
  ...
  set +e
  {
    ...
    echo "-- next PIDs from this runner --"
    if [ -n "${NEXT_PID:-}" ]; then
...
INBUCKET_URL="$(printf '%s' "$STATUS_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("INBUCKET_URL", "http://127.0.0.1:54324"))')"
...
if lsof -i ":${E2E_NEXT_PORT}" -sTCP:LISTEN >/dev/null 2>&1; then
  echo "[e2e-otp] port ${E2E_NEXT_PORT} already in use — aborting" >&2
  exit 1
fi
...
if [ "$READY" -ne 1 ]; then
  echo "[e2e-otp] next dev never became ready within 90s" >&2
...
# Warm-up: Turbopack compiles route handlers lazily on first hit. The
# suite would otherwise race the first `/api/v2/onboarding` compile
# against the browser-side `expectAuthenticatedChat` and see an empty
# workspace mapping. A single unauthenticated POST (returns 401 opaque)
# is enough to force compilation before Playwright starts.
log "warming up /api/v2/onboarding (unauthenticated)"
curl -sS -o /dev/null -w "[e2e-otp] warmup /api/v2/onboarding = %{http_code} (%{time_total}s)\n" \
  -X POST "${E2E_BASE_URL}/api/v2/onboarding" >&2 || true

log "running Playwright (chromium, otp-signin.spec.ts)"
set +e
...
SPABLA_E2E_INBUCKET_URL="${INBUCKET_URL}" \
SPABLA_E2E_NEXT_PORT="${E2E_NEXT_PORT}" \
SPABLA_E2E_REPO_ROOT="${REPO_ROOT}" \
  npx playwright test e2e/otp-signin.spec.ts --project chromium
PLAYWRIGHT_EC=$?
set -e
