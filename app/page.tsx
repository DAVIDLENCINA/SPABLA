...
const SLIDES = [
  {
    img: "/hero1.jpg",
    title: "Habla con el mundo.",
    sub: "Videollamadas con traducción de voz en tiempo real.",
  },
  {
    img: "/hero2.jpg",
    title: "Entiende cualquier idioma.",
    sub: "Comunícate al instante en cualquier país.",
  },
  {
    img: "/hero3.jpg",
    title: "Tu voz. Sin fronteras.",
    sub: "Reuniones globales con traducción instantánea.",
  },
];
...
const FEATURES = [
  {
    ...
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={CY} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={CY} strokeWidth="1.7" strokeLinecap="round">
  ...
  },
  {
    ...
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={CY} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
        ...
        <rect x="2" y="7" width="6" height="11" rx="1" />
...
export default function Home() {
  ...
  function startCall() {
    ...
    router.push(`/call/${roomId}?mode=video`);
  }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::selection { background: rgba(62,198,198,.2); }
        ...
        .btn-primary {
          background: linear-gradient(135deg, ${CY}, ${CO});
        ...
        .btn-secondary:hover { background: rgba(255,255,255,.13); border-color: rgba(255,255,255,.25); }
        ...
        .store-btn {
          ...
          width: 220px; color: rgba(255,255,255,.75);
          font-size: 14px; font-weight: 600; font-family: inherit;
        }
        .store-btn:hover { background: rgba(255,255,255,.1); }
        .store-btn.active { background: ${CY}; border-color: transparent; color: #000; cursor: pointer; }
        .store-btn.active:hover { opacity: .88; }
      `}</style>
      ...
      <nav style={{
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 40px", height: 60, display: "flex", alignItems: "center", gap: 32 }}>
          ...
          <div style={{ display: "flex", gap: 24, flex: 1 }}>
            {(["Funciones", "Cómo funciona", "Seguridad", "Descargar"] as const).map((label, i) => {
              const anchors = ["features", "demo", "privacy", "descarga"];
              return (
                <button key={label} className="nav-link" onClick={() => scrollAnchor(anchors[i])}>
                  {label}
                </button>
              );
            })}
          </div>
          <button className="btn-primary" onClick={startCall} style={{ padding: "8px 20px", fontSize: 13.5 }}>
            Iniciar llamada
          </button>
        </div>
      </nav>
        ...
        <div style={{
          ...
          {SLIDES.map((_, i) => (
            <button
              ...
              style={{
                width: i === slide ? 28 : 8,
                background: i === slide ? CY : undefined,
                boxShadow: i === slide ? `0 0 12px rgba(62,198,198,.65)` : "none",
              }}
            />
          ))}
        </div>
      ...
      <section style={{ background: BG, padding: "96px 40px", textAlign: "center", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
        <div style={{ maxWidth: 580, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", marginBottom: 32 }}>
            <button className="btn-primary" onClick={startCall}>Iniciar llamada</button>
            <button className="btn-secondary" onClick={() => scrollAnchor("descarga")}>Descargar app</button>
          </div>
      ...
      <section id="features" style={{ padding: "80px 40px", background: BG, borderBottom: "1px solid rgba(255,255,255,.05)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".15em", color: "rgba(255,255,255,.25)", textTransform: "uppercase", textAlign: "center", marginBottom: 12 }}>
            Por qué SPABLA
          </p>
          <h2 style={{ fontSize: "clamp(24px, 3.5vw, 38px)", fontWeight: 800, letterSpacing: "-.035em", textAlign: "center", marginBottom: 52 }}>
      ...
      <section id="descarga" style={{ padding: "96px 40px", background: "rgba(255,255,255,.015)", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
        <div style={{ maxWidth: 440, margin: "0 auto", textAlign: "center" }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".15em", color: "rgba(255,255,255,.25)", textTransform: "uppercase", marginBottom: 12 }}>
            App móvil
          </p>
          <h2 style={{ fontSize: "clamp(28px, 4vw, 46px)", fontWeight: 800, letterSpacing: "-.04em", marginBottom: 16, lineHeight: 1.1 }}>
            Llévalo contigo.
          </h2>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
            {[
              ...
              },
              {
                ...
                icon: (
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
                    <path d="M3.18 23.76c.27.15.59.19.94.08l11.7-6.74-2.51-2.52-10.13 9.18zM20.4 10.55l-2.56-1.49-2.84 2.57 2.84 2.7 2.59-1.5c.74-.43.74-1.85-.03-2.28zM2.29.29C2.1.52 2 .86 2 1.29v21.42c0 .43.11.77.3 1l.09.07 12-11.88v-.28L2.29.29zM14.32 8.21L3.18.25l-.04-.02 10.17 9.24 1.01-.96-.93-.3z" />
                  </svg>
                ),
              },
              {
                label: "Usar en la Web",
                sub: "Gratis · Sin instalar nada",
                active: true,
                icon: (
              ...
              <button
                key={b.label}
                className={`store-btn${b.active ? " active" : ""}`}
              ...
              >
                ...
                <span style={{ textAlign: "left" }}>
                  ...
                  <span style={{ display: "block", fontWeight: 700, color: b.active ? "#000" : "rgba(255,255,255,.75)" }}>{b.label}</span>
                </span>
              </button>
            ))}
          </div>
