...
export default function CallPage() {
  ...
  // Dos captions independientes: local y remote
  ...
  async function translate(text: string, from: LangCode, to: LangCode) {
  ...
  function showLocal(next: Caption) {
    ...
    if (!next.partial) {
      hideLocalRef.current = window.setTimeout(() => setLocalCaption(null), 6500);
    }
  }
  ...
  function showRemote(next: Caption) {
    ...
    if (!next.partial) {
      hideRemoteRef.current = window.setTimeout(() => setRemoteCaption(null), 6500);
    }
  }
  ...
  useEffect(() => {
    ...
    async function startCall() {
      ...
      socket.on("ice-candidate", async (data) => { try { await pc.addIceCandidate(data.candidate); } catch {} });
  ...
  return (
    <main style={{
      background: "#000", width: "100vw", height: "100svh",
      ...
      <video ref={remoteVideoRef} autoPlay playsInline style={{
        ...
        objectFit: "cover", opacity: hasRemote ? 1 : 0, transition: "opacity .5s", zIndex: 1,
      }}/>
      ...
      <div style={{
        ...
        background: "linear-gradient(to bottom, rgba(0,0,0,.28) 0%, rgba(0,0,0,.0) 30%, rgba(0,0,0,.60) 68%, rgba(0,0,0,.97) 100%)",
      }}/>
      ...
      {!hasRemote && (
        <section style={{
          ...
          <div>
            <h1 style={{ color: "#fff", fontSize: "clamp(28px, 7vw, 56px)", fontWeight: 900, margin: 0, letterSpacing: "-.04em" }}>
              Esperando participante
            </h1>
            <p style={{ color: "rgba(255,255,255,.6)", fontSize: "clamp(14px, 3.5vw, 22px)", marginTop: 12 }}>
              Comparte el enlace para empezar
            </p>
          </div>
        </section>
      )}
      ...
      <div style={{
        ...
        borderRadius: 999, padding: "8px 14px", backdropFilter: "blur(18px)",
      }}>
      ...
      {/* ── SUBTÍTULOS — dos entradas independientes ── */}
      <div style={{
        position: "absolute", left: 0, right: 0,
        bottom: "clamp(130px, 19vh, 200px)",
        ...
        {[localCaption, remoteCaption].map((cap) => {
          ...
          const flagSrc = LANGS[cap.to]?.flag ?? LANGS[cap.from].flag;
          return (
            <div key={cap.speaker} style={{ animation: "captionIn .22s ease-out both" }}>
              {/* nombre · hora */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ color: accent, fontWeight: 800, fontSize: "clamp(13px, 3vw, 16px)", letterSpacing: ".01em" }}>
                  {isLocal ? "Tú" : "Participante"}
                </span>
                <span style={{ color: "#8E8E93", fontSize: 9 }}>•</span>
                <span style={{ color: "#8E8E93", fontSize: "clamp(11px, 2.5vw, 14px)", fontVariantNumeric: "tabular-nums" }}>
                  {cap.time}
                </span>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "clamp(10px, 2.5vw, 14px)" }}>
                <img src={flagSrc} alt={cap.to} style={{
                  width: "clamp(30px, 7vw, 42px)", height: "clamp(30px, 7vw, 42px)",
                  borderRadius: 9, objectFit: "cover", flexShrink: 0,
                  boxShadow: "0 4px 14px rgba(0,0,0,.50)",
                }}/>
                <span style={{
                  color: "#fff",
                  ...
                  letterSpacing: "-.03em",
                  textShadow: "0 3px 20px rgba(0,0,0,.70)",
              ...
              {!cap.partial && cap.original !== cap.translated && (
                <div style={{
                  display: "flex", alignItems: "center", gap: 8,
                  marginTop: 6, paddingLeft: "clamp(40px, 9.5vw, 56px)",
                  color: "rgba(255,255,255,.45)", fontSize: "clamp(12px, 2.8vw, 16px)", fontStyle: "italic",
                }}>
                  <img src={LANGS[cap.from].flag} alt={cap.from} style={{
                    width: "clamp(18px, 4vw, 24px)", height: "clamp(18px, 4vw, 24px)",
                    borderRadius: 5, objectFit: "cover", flexShrink: 0, opacity: .75,
                  }}/>
              ...
              {/* línea de color */}
              <div style={{
                marginTop: 8, height: 2, borderRadius: 999,
                background: isLocal
                  ? "linear-gradient(90deg, transparent, #00E5FF, transparent)"
                  : "linear-gradient(90deg, transparent, #FF6B8A, transparent)",
      ...
      {/* ── CONTROLES FLOTANTES — sin cápsula ── */}
      <nav style={{
        position: "absolute",
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7 }}>
          <button onClick={hangUp} style={{
            ...
            border: "none",
            boxShadow: "0 0 0 1px rgba(255,92,106,.40), 0 0 32px rgba(255,40,60,.60), 0 8px 28px rgba(0,0,0,.65)",
            display: "flex", alignItems: "center", justifyContent: "center",
            cursor: "pointer",
          }}>
        ...
        <CtrlBtn onClick={switchLang} label="Idioma">
...
function CtrlBtn({ onClick, label, danger, children }: {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7, flexShrink: 0 }}>
      <button onClick={onClick} style={{
        ...
        borderRadius: "50%",
        background: danger ? "rgba(255,50,70,.80)" : "rgba(255,255,255,.10)",
        border: `1.5px solid ${danger ? "rgba(255,60,80,.55)" : "rgba(255,255,255,.18)"}`,
      ...
      <span style={{ fontSize: 10, color: "rgba(255,255,255,.42)", letterSpacing: ".03em", whiteSpace: "nowrap" }}>
        {label}
      </span>
    </div>
  );
