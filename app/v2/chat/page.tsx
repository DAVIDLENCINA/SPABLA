...
// UX-02 · Los tokens visuales productivos viven en `./styles.ts`
// (promovidos desde UX-01-R2). Nunca importamos `app/v2/design/**`.
      ...
      {!canOperate ? (
        <section
          ...
          style={{
            ...
            padding: `${chatSpace.xxl}px ${chatSpace.lg}px`,
        ...
        >
          {/*
            UX-02 · Cuando NO hay sesión, `UnauthGate` ya invita al
            usuario (bien con "Restaurando tu sesión…", bien con el
            formulario de acceso). Repetir aquí el mismo mensaje
            producía duplicación visual — la timeline queda como
            espacio vacío hasta que hay contexto real que comunicar.
            Los mensajes propios de `session` sí se muestran para
            distinguir bootstrap loading / retry / conversación
            indisponible (Q2 §9 máquina de estados).
          */}
          <span>
            {!session
              ? ""
        ...
        {messages.length === 0 && (
          <div
            style={{
              ...
              alignItems: "center",
              justifyContent: "center",
              padding: `${chatSpace.xl}px ${chatSpace.lg}px`,
              textAlign: "center",
              color: chatColor.textSecondary,
            }}
            aria-live="polite"
          >
            <p style={{ margin: 0, fontSize: chatFont.size.base, fontWeight: chatFont.weight.semibold, color: chatColor.textPrimary }}>
              Aún no hay mensajes.
            </p>
            <p style={{ margin: `${chatSpace.xs}px 0 0`, fontSize: chatFont.size.sm, color: chatColor.textMuted }}>
              Escribe el primero para comenzar la conversación.
            </p>
          </div>
        )}
        {messages.length > 0 && (
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: chatSpace.sm, minWidth: 0 }}>
            {messages.map((m) => {
                  ...
                  >
                    <div
                      style={{
                        ...
                        background: isOwn ? chatColor.bubbleSelfBg : chatColor.bubbleOtherBg,
                        color: isOwn ? chatColor.bubbleSelfText : chatColor.bubbleOtherText,
                        boxShadow: isOwn ? "none" : "0 1px 2px rgba(15, 23, 42, 0.05)",
                        minWidth: 0,
                    ...
                    >
                      ...
                      {showTranslation && (
                        <div
                          style={{
                            marginTop: chatSpace.sm,
                            paddingTop: chatSpace.sm,
                            borderTop: `1px solid ${isOwn ? "rgba(11, 15, 25, 0.16)" : chatColor.border}`,
                            fontSize: chatFont.size.sm,
                            color: isOwn ? "rgba(11, 15, 25, 0.72)" : chatColor.textSecondary,
                          }}
                        >
