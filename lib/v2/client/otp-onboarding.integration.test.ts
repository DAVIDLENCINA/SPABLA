...
afterAll(async () => {
  ...
  try {
    if (createdUserIds.size > 0) {
      ...
      await pg.query(`DELETE FROM spabla_v2.tenant_memberships WHERE actor_id = ANY($1::uuid[])`, [ids]);
      await pg.query(`DELETE FROM spabla_v2.actor_personal_workspace WHERE actor_id = ANY($1::uuid[])`, [ids]);
    }
    if (createdTenantIds.size > 0) {
      const ids = Array.from(createdTenantIds);
      await pg.query(`DELETE FROM spabla_v2.tenant_memberships WHERE tenant_id = ANY($1::uuid[])`, [ids]);
      await pg.query(`DELETE FROM spabla_v2.tenants WHERE id = ANY($1::uuid[])`, [ids]);
    }
  } finally {
...
describe.runIf(ENABLED)("otp → onboarding real (Supabase local + Mailpit + handler real)", () => {
  test("flujo completo · request → email → verify → handler REAL /api/v2/onboarding → 200 + estado SQL correcto", async () => {
    ...
    if (created.data.user) createdUserIds.add(created.data.user.id);
  ...
  test("guarda anti-usurpación · access_token de otro actor NO permite operar sobre el primero", async () => {
    ...
    createdUserIds.add(cA.data.user!.id);
    createdUserIds.add(cB.data.user!.id);
    ...
    createdTenantIds.add(oA.body.tenantId!);
    createdTenantIds.add(oB.body.tenantId!);
